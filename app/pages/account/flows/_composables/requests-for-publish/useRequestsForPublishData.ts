...
import { getPublicFlowsOnModeration } from '~/domain/flow/api/public-flows.api';
...
function useRequestsForPublishData(options?: {
  ...
  const getTableData = async () => {
    try {
      ...
      const response = await getPublicFlowsOnModeration(params);

      data.value = response.data;
      meta.value = response.meta.pagination;
    } catch (e) {
      toastError('Ошибка при получении запросов на публикацию');
      showRequestError(e);
    } finally {
      loading.value = false;
    }
...
export default useRequestsForPublishData;
