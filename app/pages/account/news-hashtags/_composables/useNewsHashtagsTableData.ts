...
function useNewsHashtagsTableData(options?: {
  ...
  const data = ref<NewsHashtag[] | null>(null);
  ...
  const getTableData = async (sortableProp?: string, sortableDir?: string) => {
    try {
      ...
      const response = await getNewsHashtags(params);

      data.value = response.data;
      meta.value = response.meta.pagination;
    } catch (e) {
      toastError('Ошибка при получении хэштегов новостей');
      showRequestError(e);
    } finally {
      loading.value = false;
    }
...
export default useNewsHashtagsTableData;
