...
import { getSocialPostsRequest } from '~/domain/social-post/api/social-post.api';
...
function useSocialPostsTableData(options?: {
  ...
  const getTableData = async (sortableProp?: string, sortableDir?: string) => {
    try {
      ...
      const response = await getSocialPostsRequest(params);

      data.value = response.data;
      meta.value = response.meta.pagination;
    } catch (e) {
      toastError('Ошибка при получении постов в соц. сетях');
      showRequestError(e);
    } finally {
      loading.value = false;
    }
...
export default useSocialPostsTableData;
